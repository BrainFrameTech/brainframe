...
class CrdtNoteWriter implements NoteWriter {
  ...
  Future<void> _write(String path, String text) async {
    ...
    // browser can create one, and until the scan lands (step 11) this is the
    // only thing that notices. Seeding with the buffer rather than with the
    // file's old content is deliberate: the buffer is what the user means, and
    // seeding with anything else would make the user's first keystroke an edit
    // against a document they never saw.
    if (row == null) {
